  ...
  <div class="sidebar-content">
    ...
    <div class="depot-list">
      <div *ngFor="let upgeo of data" class="depot-item">
        <!-- UPGEO Header -->
        <button class="section-btn depot-btn" [ngClass]="{ 'active': selectedUpgeoId === upgeo.upgeoId }" (click)="handleUpgeoClick(upgeo)">
          ...
          <div class="section-arrow">
            <i class="fas" [ngClass]="openUpgeos[upgeo.upgeoId] ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
          </div>
        </button>
        ...
        <div class="dropdown-content" [ngClass]="{ 'open': openUpgeos[upgeo.upgeoId] }">
          <div *ngFor="let geo of upgeo.geoareas" class="role-item">
            <button class="section-btn role-btn" [ngClass]="{ 'active': selectedGeoId === geo.geoId }" (click)="handleGeoClick(geo)">
              ...
              <div class="section-arrow">
                <i class="fas" [ngClass]="openGeos[geo.geoId] ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
              </div>
            </button>
            ...
            <div class="dropdown-content" [ngClass]="{ 'open': openGeos[geo.geoId] }">
              ...
              <div *ngFor="let itinerary of geo.itineraries" class="tournee-item">
                <button class="section-btn tournee-btn" [ngClass]="{ 'active': selectedItineraryId === itinerary.itineraryId }" (click)="handleItineraryClick(itinerary)">
                  ...
                  <div class="section-arrow">
                    <i class="fas" [ngClass]="openItineraries[itinerary.itineraryId] ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
                  </div>
                </button>
                ...
                <div class="dropdown-content" [ngClass]="{ 'open': openItineraries[itinerary.itineraryId] }">
                  <div class="client-scroll">
                   <div *ngFor="let client of itinerary.clients" class="client-item">
                    <button class="section-btn client-btn" [ngClass]="{ 'active': selectedClientId === client.id }" (click)="handleClientClick(client)">
...
export class SidebarComponent implements OnInit {
  ...
  loadSidebarData() {
    this.http.get<Upgeo[]>('http://localhost:8000/sidebar-data')
      .subscribe({
        next: (response) => {
          this.data = response;
          console.log('Sidebar data loaded:', this.data);
        },
        error: (err) => {
  ...
  toggleUpgeo(upgeoId: string) {
    this.openUpgeos = {
      ...this.openUpgeos,
      [upgeoId]: !this.openUpgeos[upgeoId]
    };
  }
  ...
  toggleItinerary(itineraryId: string) {
    this.openItineraries = {
      ...this.openItineraries,
      [itineraryId]: !this.openItineraries[itineraryId]
    };
  }
  ...
  handleGeoClick(geo: GeoArea) {
    ...
    this.selectedItineraryId = null;
    this.selectedClientId = null;
    this.onSelect.emit({ upgeoId: this.selectedUpgeoId, geo });
  }
  ...
  optimizeVisit() {
  const upgeo = this.data.find((u: Upgeo) => u.upgeoId === this.selectedUpgeoId);
  if (!upgeo) return;

  const geo = upgeo.geoareas.find((g: GeoArea) => g.geoId === this.selectedGeoId);
  if (!geo) return;

  const itinerary = geo.itineraries.find((i: Itinerary) => i.itineraryId === this.selectedItineraryId);
  if (!itinerary || itinerary.clients.length === 0) return;

  const payload = {
    lat: itinerary.clients[0].lat,
    lon: itinerary.clients[0].lon,
    itinerary: {
   ...
   fetch('http://localhost:8000/optimize-visit/', {
    ...
    .then(blob => {
      ...
      .catch(error => {
        console.error('Error optimizing visit:', error);
      });
  }
}
